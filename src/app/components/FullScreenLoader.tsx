...
// Original Butterfly Component from Uiverse.io
...
// Configuration for multiple crossing butterflies
const butterflies = [
  { id: 1, duration: 8, delay: 0, yStart: '20vh', yEnd: '80vh', scale: 1 },
  { id: 2, duration: 10, delay: 0.5, yStart: '70vh', yEnd: '30vh', scale: 0.8 },
  { id: 3, duration: 9, delay: 1.5, yStart: '50vh', yEnd: '10vh', scale: 1.2 },
  { id: 4, duration: 11, delay: 0.2, yStart: '10vh', yEnd: '90vh', scale: 0.9 },
  { id: 5, duration: 12, delay: 0.8, yStart: '80vh', yEnd: '20vh', scale: 1.1 },
];
...
export function FullScreenLoader() {
  return (
    <div className="fixed inset-0 z-[100] bg-white flex items-center justify-center overflow-hidden">
      ...
      {/* Multiple moving butterflies crossing the screen */}
      {butterflies.map((b, i) => {
        // Alternate directions
        const isLeftToRight = i % 2 === 0;
        const initialX = isLeftToRight ? '-20vw' : '120vw';
        const targetX = isLeftToRight ? '120vw' : '-20vw';
        
        // If moving right to left, flip horizontally so it faces the direction
        const flip = isLeftToRight ? 1 : -1;

        return (
          <motion.div
            key={b.id}
            className="absolute"
            initial={{ x: initialX, y: b.yStart, scaleX: flip, scaleY: b.scale }}
            animate={{ 
              x: [initialX, targetX],
              y: [b.yStart, b.yEnd, b.yStart] 
            }}
            transition={{ 
              x: { duration: b.duration, repeat: Infinity, ease: "linear", delay: b.delay },
              y: { duration: b.duration * 0.8, repeat: Infinity, repeatType: "mirror", ease: "easeInOut", delay: b.delay }
            }}
          >
            <ButterflyLoader />
          </motion.div>
